...
NEW_DATA = 'data/NEW-WiC/'
COLAB_PREFIX = '/content/PRISM/'

def read_data_related(path, read_tags=False):
    with open(path) as f:
    ...
    if read_tags:
        ...
        df['tag'] = df['tag'].replace({'T': 2, 'R': 1, 'F': 0})
...
def read_data_wic(path, read_tags=False):
    ...
    if read_tags:
        ...
        df['tag'] = df['tag'].replace({'T': 2, 'F': 0})        

    df['pos'] = df['pos'].replace({'N': 'NOUN', 'V': 'VERB'})

    id_string = path[path.rfind('/') + 1: path.find('.data')]
    df['id'] = df.index
    df['id'] = df['id'].apply(lambda id: id_string + '_wic.en-en.' + str(id))
